  ...
  return (
    <section className="py-12">
      <div className="container-pad">
        ...
        <div className="clay mt-8 flex gap-2 overflow-x-auto rounded-clay-sm p-2">
          {tabs.map(([key, Icon, label]) => (
            <button
              key={key}
              className={`inline-flex items-center gap-2 rounded-clay-sm px-4 py-3 text-sm font-bold transition ${
                activeTab === key
                  ? 'bg-brand-gradient text-white shadow-clay-sm'
...
function DataTable({ columns, rows }) {
  return (
    <div className="clay mt-8 overflow-hidden rounded-clay">
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-sm">
          <thead className="text-xs uppercase tracking-widest text-slate-500 dark:text-slate-400" style={{ background: 'var(--clay-surface-alt)' }}>
            <tr>
              {columns.map((column) => (
